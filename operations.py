...
class Operations():
    ...
    gamma, theta, vega, sell_leg_bid, buy_leg_ask = .056, .400, .030, .30, .18
    ...
    def checkCriteria(self):
        ...
        for index, row in self.option_chain.iterrows():
            ...
            if (row["bid"] == None or row["bid"] == -1 or row["vega"] == None or row["ask"] == None or row["ask"] == -1):
                continue
            if (row["bid"] > self.sell_leg_bid and row["vega"] >= self.vega and row["gamma"] <= self.gamma and abs(row["theta"]) <= self.theta):
                ...
                for index, row in self.option_chain[::-1].iterrows():
                    if (row["ask"] == None):
                        continue
                    if (row["ask"] < self.buy_leg_ask): #buy wing shouldn't be more than 18 cents
                        spreads.update({str(sell_leg) + ', ' + str(row["strike"]) : [0, 0, 0]})
        print("displaying spread dict being returned in checkCriteria")
        print(spreads)
        return(spreads)
